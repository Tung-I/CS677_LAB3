...
LOOKUP_TEST_SET1 = [
    ...
    "/lookup?stock=SPX",
    "/lookup?stock=OEX",
    # stock name not found -> error 404, stock not found x 4
    "/lookup?stock=GOOGGGGGGG",
    "/lookup?stock=APPPPPL",
    "/lookup?stock=PSXXXXXXXXXX",
...
def main(args):
    ...
    # Run the test cases
    for test_set in [LOOKUP_TEST_SET1, ORDER_TEST_SET1, LOOKUP_TEST_SET2, ORDER_TEST_SET2, ORDER_TEST_SET3]:
    # for test_set in [LOOKUP_TEST_SET2]:
        for request in test_set:
            url = f'http://{args.host}:{args.port}{request}'
            if request.startswith("/lookup") or request.startswith("/order?order_number"):
